...
describe('Class API', () => {
    ...
    beforeAll(async () => {
        ...
        trainerId = 999992;
        userId = 999991;
        adminToken = (0, jwt_1.generateToken)({ id: adminId, email: 'admin@class.test.com', role: client_1.Role.ADMIN });
        trainerToken = (0, jwt_1.generateToken)({ id: trainerId, email: 'trainer@class.test.com', role: client_1.Role.TRAINER });
        userToken = (0, jwt_1.generateToken)({ id: userId, email: 'user@class.test.com', role: client_1.Role.USER });
        // Create admin
    ...
    afterAll(async () => {
        ...
        if (classId) {
            await prisma_1.default.classEnrollment.deleteMany({ where: { classId } });
            await prisma_1.default.class.deleteMany({ where: { id: classId } });
        }
        await prisma_1.default.classCategory.delete({ where: { id: categoryId } }).catch(() => { });
        // Delete all classes created by these users first
        await prisma_1.default.classEnrollment.deleteMany({ where: { userId: { in: [adminId, trainerId, userId] } } }).catch(() => { });
        await prisma_1.default.class.deleteMany({ where: { OR: [{ createdById: { in: [adminId, trainerId, userId] } }, { trainerId: { in: [adminId, trainerId, userId] } }] } }).catch(() => { });
        await prisma_1.default.user.deleteMany({ where: { id: { in: [adminId, trainerId, userId] } } }).catch(() => { });
    });
    describe('POST /api/classes', () => {
        ...
        it('should return 400 if requiredRole is invalid', async () => {
            const response = await (0, supertest_1.default)(app)
                ...
                startTime: new Date(Date.now() + 86400000).toISOString(),
                endTime: new Date(Date.now() + 90000000).toISOString(),
                requiredRole: client_1.Role.ADMIN, // invalid role
            })
                .expect(400);
        ...
        it('should create class successfully', async () => {
            ...
            const response = await (0, supertest_1.default)(app)
                ...
                capacity: 20,
                requiredRole: client_1.Role.USER_BRONZE,
            })
                .expect(201);
    ...
    describe('POST /api/classes/:classId/enroll', () => {
        ...
        it('should return 400 if class has started', async () => {
            ...
            const pastClass = await prisma_1.default.class.create({
                data: {
                    title: 'Past Class',
            ...
            await prisma_1.default.class.delete({ where: { id: pastClass.id } });
        });
        it('should return 400 if class is full', async () => {
            if (classId) {
                // Create a class with capacity 1
                const limitedClass = await prisma_1.default.class.create({
                    data: {
                        title: 'Limited Class',
                ...
                const otherToken = (0, jwt_1.generateToken)({ id: otherUser.id, email: otherUser.email, role: client_1.Role.USER });
                const response = await (0, supertest_1.default)(app)
                    .post(`/api/classes/${limitedClass.id}/enroll`)
                    .set('Authorization', `Bearer ${otherToken}`)
                    .expect(400);
                expect(response.body.message).toContain('already full');
                await prisma_1.default.classEnrollment.deleteMany({ where: { classId: limitedClass.id } });
                await prisma_1.default.class.delete({ where: { id: limitedClass.id } });
        ...
        it('should return 403 if user role does not meet requiredRole', async () => {
            if (classId) {
                ...
                const restrictedClass = await prisma_1.default.class.create({
                    data: {
                        title: 'Restricted Class',
                        startTime: new Date(Date.now() + 86400000),
                        endTime: new Date(Date.now() + 90000000),
                        requiredRole: client_1.Role.USER_GOLD,
                        createdById: adminId,
                        trainerId,
                    },
                });
                ...
                await prisma_1.default.class.delete({ where: { id: restrictedClass.id } });
            }
        });
        it('should enroll user successfully', async () => {
            if (classId) {
                // Create a class without restrictions
                const openClass = await prisma_1.default.class.create({
                    data: {
                        title: 'Open Class',
                        startTime: new Date(Date.now() + 86400000),
                        endTime: new Date(Date.now() + 90000000),
                ...
                await prisma_1.default.classEnrollment.deleteMany({ where: { classId: openClass.id } });
                await prisma_1.default.class.delete({ where: { id: openClass.id } });
            }
        });
    ...
    describe('GET /api/classes/trainer/:trainerId', () => {
        ...
        it('should return 403 if trainer tries to view another trainer\'s classes', async () => {
            const otherTrainer = await prisma_1.default.user.create({
                data: {
            ...
            const otherToken = (0, jwt_1.generateToken)({ id: otherTrainer.id, email: otherTrainer.email, role: client_1.Role.TRAINER });
            const response = await (0, supertest_1.default)(app)
                .get(`/api/classes/trainer/${trainerId}`)
                .set('Authorization', `Bearer ${otherToken}`)
                .expect(403);
            expect(response.body.message).toContain('your own classes');
            await prisma_1.default.user.delete({ where: { id: otherTrainer.id } });
        });
    ...
    describe('PUT /api/classes/:classId', () => {
        let updateClassId;
        beforeEach(async () => {
            const clazz = await prisma_1.default.class.create({
                data: {
        ...
        afterEach(async () => {
            if (updateClassId) {
                await prisma_1.default.classEnrollment.deleteMany({ where: { classId: updateClassId } });
                await prisma_1.default.class.delete({ where: { id: updateClassId } });
            }
        });
        it('should return 401 if no token', async () => {
            const response = await (0, supertest_1.default)(app)
    ...
    describe('DELETE /api/classes/:classId', () => {
        ...
        it('should delete class with enrollments', async () => {
            const clazzWithEnrollments = await prisma_1.default.class.create({
                data: {
