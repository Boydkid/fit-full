...
async function handleWebhook(req, res) {
    ...
    try {
        if (event.type === "checkout.session.completed") {
            ...
            if (!purchase)
                return res.json({ received: true }); // ไม่เจอ—ข้าม
            ...
            const plan = Object.values(stripePlans_1.STRIPE_PRICE_TO_PLAN).find(p => p.role === targetRole);
            if (plan) {
...
async function verifySession(req, res) {
    try {
        ...
        let purchase = (await prisma.membershipPurchase.findFirst({
